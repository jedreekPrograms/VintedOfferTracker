...
class PriceProbeAssignmentTest {
    ...
    void setUp() {
        ...
        service = new PriceProbeService(botRepository, listingRepository, priceProbeRepository);
    }

    @Test
    void assignmentUsesLowerPriceAndExplicitCurrency() {
        Bot probeBot = bot(1L);
        Bot sourceBot = bot(2L);
        Listing source = listing(100L, sourceBot);

        when(botRepository.findById(1L)).thenReturn(Optional.of(probeBot));
        when(listingRepository.findByStatusInOrderByIdAsc(any())).thenReturn(List.of(source));
        when(listingRepository.findByIdForUpdate(100L)).thenReturn(Optional.of(source));
        when(priceProbeRepository.existsByProbeBot_IdAndSourceListing_Id(1L, 100L)).thenReturn(false);
        when(priceProbeRepository.countBySourceListing_Id(100L)).thenReturn(0L);
        when(priceProbeRepository.saveAndFlush(any(PriceProbe.class))).thenAnswer(invocation -> {
            PriceProbe probe = invocation.getArgument(0);
            probe.setId(500L);
            return probe;
        });

        var assignment = service.claimNext(1L).orElseThrow();
