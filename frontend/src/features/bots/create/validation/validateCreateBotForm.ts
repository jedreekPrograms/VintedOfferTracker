...
export function validateCreateBotForm({
    ...
    if (normalizedBotName.length === 0) {
        return invalid("Wpisz nazwę bota.");
    }

    const normalizedEmail = form.email.trim();
    if (normalizedEmail.length === 0) {
        return invalid("Wpisz e-mail konta Vinted.");
    }
    if (requirePassword && form.password.length === 0) {
        return invalid("Wpisz hasło konta Vinted.");
    }

    if (selectedCategory === null) {
        return invalid("Wybierz kategorię.");
    }
    if (selectedBrand === null) {
        return invalid("Wybierz markę.");
    }
    if (selectedModel === null) {
        return invalid("Wybierz model ze słownika.");
    }
    if (selectedModel.brandId !== selectedBrand.id) {
        return invalid("Wybrany model nie należy do wybranej marki.");
    }

    const targetMode = selectedModel.targetMode;
    const validatedModel = targetMode === "VINTED_MODEL"
        ? selectedModel
        : null;
    const validatedSearchQuery = targetMode === "SEARCH_QUERY"
        ? normalizeText(selectedModel.name)
        : null;

    const minPriceResult = parseRequiredNonNegativeNumber(
        form.minPrice,
        "Minimalna cena jest nieprawidłowa.",
    );
    if (!minPriceResult.valid) {
        return invalid(minPriceResult.errorMessage);
    }

    const maxPriceResult = parseRequiredPositiveNumber(
        form.maxPrice,
        "Maksymalna cena jest nieprawidłowa.",
    );
    if (!maxPriceResult.valid) {
        return invalid(maxPriceResult.errorMessage);
    }

    const minPrice = minPriceResult.value;
    const maxPrice = maxPriceResult.value;
    if (minPrice > maxPrice) {
        return invalid("Minimalna cena nie może być większa od maksymalnej.");
    }
    ...
    if (!Number.isInteger(dailyNegotiationBudget)
    ...
    if (form.negotiationSteps.length === 0) {
        return invalid("Bot musi mieć przynajmniej jeden krok negocjacji.");
    }
    ...
    for (let index = 0; index < form.negotiationSteps.length; index += 1) {
        ...
        const offerResult = parseRequiredPositiveNumber(
            step.offerPrice,
            `Krok ${stepNumber}: cena oferty jest nieprawidłowa.`,
        );
        if (!offerResult.valid) {
            return invalid(offerResult.errorMessage);
        }
        ...
        if (!counterResult.valid) {
            return invalid(counterResult.errorMessage);
        }
        ...
        if (normalizedMessage.length === 0) {
            return invalid(`Krok ${stepNumber}: wiadomość jest wymagana.`);
        }

        const rejectionWait = validateWait(
            step.rejectionAction,
            step.rejectionWaitHours,
            `Krok ${stepNumber}: czas po odrzuceniu`,
        );
        if (!rejectionWait.valid) {
            return invalid(rejectionWait.errorMessage);
        }
        ...
        if (!counterFallbackWait.valid) {
            return invalid(counterFallbackWait.errorMessage);
        }
        ...
        for (let ruleIndex = 0; ruleIndex < step.counterOfferRules.length; ruleIndex += 1) {
            ...
            if (!thresholdResult.valid) {
                return invalid(thresholdResult.errorMessage);
            }

            const threshold = thresholdResult.value;
            if (threshold > 100) {
                return invalid(
                    `Krok ${stepNumber}, reguła ${ruleIndex + 1}: próg obniżki nie może przekraczać 100%.`,
                );
            }
            if (usedThresholds.has(threshold)) {
                return invalid(
                    `Krok ${stepNumber}: próg ${threshold}% występuje więcej niż raz.`,
                );
            }
            usedThresholds.add(threshold);

            const waitResult = validateWait(
                rule.action,
                rule.waitHours,
                `Krok ${stepNumber}, reguła od ${threshold}%`,
            );
            if (!waitResult.valid) {
                return invalid(waitResult.errorMessage);
            }

            validatedRules.push({
                minimumDiscountPercent: threshold,
                action: rule.action,
                waitHours: waitResult.value,
            });
        }

        validatedRules.sort(
            (left, right) => left.minimumDiscountPercent - right.minimumDiscountPercent,
        );
...
function validateWait(
    ...
    if (!Number.isInteger(hours)
        || hours < 1
        || hours > MAX_RESPONSE_WAIT_HOURS) {
        return {
            valid: false,
            errorMessage:
                `${label}: wpisz pełną liczbę godzin od 1 do ${MAX_RESPONSE_WAIT_HOURS}.`,
        };
    }
