...
public class NegotiationDecisionService {
    ...
    public NegotiationDecision decide(
        ...
        Objects.requireNonNull(
                snapshot,
                "Conversation snapshot cannot be null"
        );
        ...
        NegotiationDecision decision =
                switch (snapshot.result()) {
                    ...
                    case PENDING ->
                            NegotiationDecision.waitForSeller();

                    case ACCEPTED ->
                            NegotiationDecision
                                    .actionRequiredAfterAcceptance();
    ...
    private NegotiationDecision decideAfterRejection(
        ...
        if (nextStep.isPresent()) {
            ...
            return NegotiationDecision.sendNextStep(
                    nextStep.get(),
                    null,
                    "The seller rejected the latest offer "
    ...
    private NegotiationDecision decideAfterSellerCounterOffer(
        ...
        if (sellerCounterOfferPrice == null) {
            ...
            throw new IllegalStateException(
                    "Snapshot is classified as SELLER_COUNTER_OFFER, "
                            + "but contains no seller counteroffer price"
            );
        ...
        BigDecimal maxAcceptedCounterOffer =
                currentStep.getMaxAcceptedCounterOffer();

        if (maxAcceptedCounterOffer != null
                && sellerCounterOfferPrice.compareTo(
                maxAcceptedCounterOffer
        ) <= 0) {

            return NegotiationDecision
                    .actionRequiredForCounterOffer(
        ...
        if (nextStep.isPresent()) {
            ...
            if (maxAcceptedCounterOffer == null) {
                ...
                reason =
                        "The current step has no accepted counteroffer limit "
                                + "and another negotiation step is available";
        ...
        return negotiationSteps.stream()
                ...
                .orElseThrow(
                        () -> new IllegalStateException(
                                "Cannot find negotiation configuration "
                                        + "for current step "
                                        + currentStepNumber
                        )
                );
    ...
    private void validateListing(
        ...
        if (!"NEGOTIATING".equals(
            ...
            throw new IllegalArgumentException(
                    "Negotiation decision can only be created "
    ...
    private void logDecision(
        ...
        log.info(
                "[DECISION] Backend listing {}, marketplace listing {}, "
                        + "current step {}, conversation result {}, "
                        + "seller counteroffer {}, decision {}, "
                        + "next step {}, reason: {}",
                listing.id(),
                listing.listingId(),
                listing.currentStep(),
